...
public class Bot {
    ...
    public String run() {
        ...
        // membangun iron curtain
        if(gameState.getPlayers().get(0).ironCurtainAvailable){
            if(!canAffordIronCurtain()){
                return command;
            }
            else{
                command = "0,0,5";
    ...
    // fungsi yang mengembalikan true bila energi cukup untuk membangun iron curtain
    private boolean canAffordIronCurtain(){
        return getEnergy(PlayerType.A) >= 100;
    }
