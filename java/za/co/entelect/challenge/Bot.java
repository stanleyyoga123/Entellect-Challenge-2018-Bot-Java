...
public class Bot {
    ...
    public String run() {
        ...
        if(gameState.gameDetails.round < 13){
            ...
            if(findMaxAttackEnemy() != -999 && canAffordBuilding(BuildingType.DEFENSE)){
                command = placeTwoBuildingFront(BuildingType.DEFENSE, findMaxAttackEnemy());
            }
    ...
    private String placeTwoBuildingFront(BuildingType buildingType, int y){
        if(isCellEmpty(gameState.gameDetails.mapWidth/2-1, y)){
            return buildCommand(gameState.gameDetails.mapWidth/2-1, y, buildingType);
        }
        else if(isCellEmpty(gameState.gameDetails.mapWidth/2-2, y)){
            return buildCommand(gameState.gameDetails.mapWidth/2-2, y, buildingType);
        }
        else{
            return "";
        }
